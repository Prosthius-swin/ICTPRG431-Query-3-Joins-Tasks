...
/*
CREATE DATABASE Query_3_Joins_Tasks;
GO
USE Query_3_Joins_Tasks;
GO
*/
...
-- 3
SELECT M.TITLE, M.RELYEAR, M.RATINGCODE, R.LONGDESC, C.COLOURCODE
FROM MOVIE9468 M
INNER JOIN RATING9468 R
ON M.RATINGCODE = R.RATINGCODE
INNER JOIN COLOURTYPE9468 C
ON M.COLOURCODE = C.COLOURCODE
WHERE M.RUNTIME < 90
ORDER BY MOVIENO ASC

-- 4
SELECT M.MOVIENO, M.TITLE, M.RATINGCODE, M.TMDB_SCORE, R.SHORTDESC
FROM MOVIE9468 M
INNER JOIN RATING9468 R
ON M.RATINGCODE = R.RATINGCODE
WHERE 
((M.RATINGCODE = 'MA' AND RUNTIME BETWEEN 100 AND 102)
OR
(M.RATINGCODE = 'G' AND RUNTIME <= 90)
OR 
(M.RATINGCODE = 'PG' AND RUNTIME BETWEEN 120 AND 125)
OR
(M.RATINGCODE = 'M' AND RUNTIME >= 180))
AND
TMDB_SCORE BETWEEN 6.5 AND 7.5
ORDER BY MOVIENO ASC
